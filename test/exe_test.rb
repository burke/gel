...
class ExeTest < Minitest::Test
  ...
  def test_exe_without_binstubs
    _, err, stat = gel_version_in_path(EXE_DIR)
  ...
  def test_exe_with_binstubs
    Dir.mktmpdir do |binstub_dir|
      %w(gel _gel-ruby-disable-gems).each do |exe|
        File.write(File.join(binstub_dir, exe), <<~BINSTUB, perm: 0755)
          #!#{RbConfig.ruby}
          #ARGV.shift
          load '#{File.join(EXE_DIR, exe)}'
        BINSTUB
      end
      out, err, stat = gel_version_in_path(binstub_dir)
      assert stat.success?, "gel version with binstubs failed: (#{out})///#{err}"
    end
  end
  ...
  def test_gel_ruby_disable_gems_from_binstub
    Dir.mktmpdir do |binstub_dir|
      ...
      File.write(File.join(binstub_dir, exe), <<~BINSTUB, perm: 0755)
        ...
        load '#{File.join(EXE_DIR, exe)}'
      BINSTUB
      _, err, stat = gel_version_in_path(binstub_dir, bin: '_gel-ruby-disable-gems')
      assert stat.success?, "failed: #{err}"
    end
  end
